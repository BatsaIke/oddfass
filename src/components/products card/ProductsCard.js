...
const ProductCard = ({ product }) => {
    ...
    return (
        <div className={`${styles.card} ${isAnimating ? styles.animating : ''}`} onClick={navigateToProductDetails}>
            <img src={product.image} alt={product.name} className={styles.image} />
            <div className={styles.details} id={product.id}>
                <h3>{product.name}</h3>
                <p>{product.description.slice(0, 90)}...</p> {/* Display truncated description */}
